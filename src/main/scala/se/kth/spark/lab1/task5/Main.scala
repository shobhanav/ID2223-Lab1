...
object Main {
  def main(args: Array[String]) {
    ...
    //Step 8 create the polynomial expansion transformer
    val polynomialExpansionT = new PolynomialExpansion().
                                setInputCol("features_3").
                                setOutputCol("polyfeatures").setDegree(2)
                                
        //step9: LinearRegression
    ...
    //Step 10: put everything together in a pipeline
    val pipeline = new Pipeline().setStages(Array(regexTokenizer,arr2Vect,lSlicer,v2d,lShifter,fSlicer,
                    polynomialExpansionT,myLR))

    //Step11: build the parameter grid by setting the values for maxIter and regParam
    ...
    println(s"RMSE after polynomial expansion and hyperparameter tuning: ${bestModelSummary.rootMeanSquaredError}")
