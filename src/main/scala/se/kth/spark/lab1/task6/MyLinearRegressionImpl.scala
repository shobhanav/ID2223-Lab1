...
object Helper {
  def rmse(labelsAndPreds: RDD[(Double, Double)]): Double = {
    ...
    val total = labelsAndPreds.map{case (a,b)=> (a-b)*(a-b)}.
    reduce(_+_)    
    return scala.math.sqrt(total/n)
  }
  ...
  def predict(weights: Vector, data: RDD[Instance]): RDD[(Double, Double)] = {
    data.map{instance => (instance.label, VectorHelper.dot(weights, instance.features))}    
  }
}
...
class MyLinearRegressionImpl(override val uid: String)
  ...
  def gradient(d: RDD[Instance], weights: Vector): Vector = {
    ...
    d.map(instance => gradientSummand(weights, instance)).reduce((a,b) => VectorHelper.sum(a, b))
  }
